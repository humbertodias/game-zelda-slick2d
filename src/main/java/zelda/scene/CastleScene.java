...
public class CastleScene extends ZeldaScene {
    ...
    private Polygon middenbovenmuur, stukmuur, stukmuur1, stukmuur2, rechtsonder, rechts, rechtsmidden, linksonder,
            links, zuil, zuil1, zuil2, zuil3, zuil4, zuil5, zuil6, zuil7, midden, midden1, pot, pot1, ding1, ding,
            ding2, ding3;
    ...
    public CastleScene(Game game, String entrance) {
        ...
        int[] xpos = {
            498, 352, 298, 217, 215, 147, 146, 145, 159, 160, 190, 193, 318, 320, 704, 704, 830, 831, 863, 865, 877,
            880, 672, 496
        };
        int[] ypos = {
            561, 563, 621, 621, 567, 561, 477, 447, 445, 175, 174, 152, 152, 167, 167, 152, 153, 174, 174, 446, 445,
            476, 528, 538
        };
        ...
        int[] axpos = {
            185, 185, 160, 160, 146, 146
        };
        int[] aypos = {
            568, 627, 628, 591, 589, 564
        };

        stukmuur = new Polygon(axpos, aypos, aypos.length);

        int[] bxpos = {
            160, 184, 185, 215, 217, 334, 335, 321, 320, 193, 191, 159
        };
        int[] bypos = {
            666, 665, 717, 715, 665, 663, 697, 695, 757, 757, 743, 743
        };

        stukmuur1 = new Polygon(bxpos, bypos, bypos.length);

        int[] cxpos = {
            530, 530, 665, 728, 727, 690, 690, 702, 704, 832, 832, 865, 865, 877, 880, 537,
        };
        int[] cypos = {
            537, 557, 559, 622, 661, 662, 694, 694, 756, 756, 735, 734, 592, 591, 476, 508,
        };

        stukmuur2 = new Polygon(cxpos, cypos, cypos.length);

        int[] dxpos = {
            532, 534, 592, 591, 670, 702, 704, 832, 832, 864, 865, 927, 927, 951, 953, 999, 1001, 1024, 1027, 1037,
            1037, 997, 996, 957, 956, 921, 921, 871, 868, 843, 837, 837, 881, 839
        };
        int[] dypos = {
            997, 959, 958, 847, 846, 814, 791, 791, 813, 813, 853, 854, 813, 814, 781, 782, 813, 813, 822, 822, 846,
            844, 787, 786, 879, 882, 878, 877, 881, 881, 882, 981, 980, 997
        };

        rechtsonder = new Polygon(dxpos, dypos, dypos.length);

        int[] expos = {
            915, 915, 998, 1000, 1112, 1110, 1074, 1073, 1085, 1086, 1075, 1074, 1108, 1109, 1045, 1047, 983, 982, 985,
            978, 804, 805, 806, 874, 873, 833, 832, 704, 704, 576, 574, 562, 561, 544, 543, 530, 529, 533, 1007, 1210,
            1210
        };
        int[] eypos = {
            996, 980, 981, 879, 876, 851, 850, 821, 821, 656, 655, 627, 624, 569, 565, 425, 422, 56, 50, 45, 48, 72, 76,
            75, 102, 103, 118, 118, 103, 102, 110, 112, 96, 96, 110, 112, 95, 31, 32, 399, 993
        };

        rechts = new Polygon(expos, eypos, eypos.length);

        int[] fxpos = {
            910, 910, 953, 953, 955, 1004, 1004, 958, 956, 995, 1034, 1038, 1040, 1026, 1023, 1000, 998, 953, 949, 928,
            927, 913, 913, 926, 926, 914
        };
        int[] fypos = {
            102, 77, 77, 465, 469, 470, 570, 569, 691, 692, 626, 631, 653, 655, 661, 663, 694, 695, 662, 661, 591, 591,
            447, 445, 103, 103
        };
        ...
        int[] gxpos = {
            494, 496, 430, 432, 433, 352, 321, 321, 192, 191, 159, 159, 145, 146, 154, 177, 182, 182, 182, 144
        };
        int[] gypos = {
            997, 958, 956, 950, 845, 844, 812, 791, 792, 814, 816, 852, 854, 876, 878, 879, 882, 952, 982, 983
        };

        linksonder = new Polygon(gxpos, gypos, gypos.length);

        int[] hxpos = {
            497, 495, 494, 481, 479, 464, 462, 450, 448, 319, 319, 193, 190, 177, 177, 111, 110, 97, 96, 110, 111, 96,
            97, 111, 112, 70, 69, 41, 41, 116, 17, 18, 496
        };
        int[] hypos = {
            100, 100, 110, 110, 102, 100, 111, 111, 99, 98, 115, 117, 108, 104, 91, 90, 101, 102, 445, 446, 590, 591,
            853, 853, 875, 876, 567, 568, 980, 981, 994, 42, 31
        };
        ...
        int[] qxpos = {
            655, 625, 623, 560, 556, 531, 529, 649, 651, 657, 656,
        };
        int[] qypos = {
            693, 693, 623, 622, 652, 652, 597, 597, 665, 670, 693,
        };

        midden = new Polygon(qxpos, qypos, qypos.length);

        int[] rxpos = {
            368, 369, 399, 401, 463, 466, 492, 496, 494, 372, 371, 366,
        };
        int[] rypos = {
            672, 694, 693, 623, 622, 649, 649, 599, 595, 595, 666, 670,
        };

        midden1 = new Polygon(rxpos, rypos, rypos.length);

        int[] sxpos = {
            434, 461, 462, 451, 452, 428, 428, 418, 417, 430, 433,
        };
        int[] sypos = {
            679, 680, 735, 733, 705, 705, 733, 733, 706, 704, 676,
        };
        ...
        int[] uxpos = {
            562, 562, 570, 572, 591, 594, 595, 605, 605, 596, 589, 590,
        };
        int[] uypos = {
            679, 732, 731, 701, 701, 705, 733, 733, 710, 708, 700, 675,
        };
